...
/**
 * Model for all AI features (recommendation, horizons, clinical read). Sonnet 4.6
 * is the default: ~5× cheaper than Opus at scale, and the deterministic
 * eligibility gate + grounding/citation guardrails (lib/ai/recommend.ts) keep the
 * output correct regardless of model. Override via SIM_MODEL (e.g. claude-opus-4-8).
 */
export const SIM_MODEL = process.env.SIM_MODEL ?? "claude-sonnet-4-6";
